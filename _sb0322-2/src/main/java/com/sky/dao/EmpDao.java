...
//		}
		...
		public List<Emp> selectAll() {
			return jdbcTemplate.query("SELECT * FROM EMP",
					(rs, rowNum) -> new Emp(rs.getInt("empno"), rs.getString("ename"), rs.getString("job"),
...
//		}
	 ...
	 // EmpDao 클래스 내
	 // 사원 번호에 해당하는 사원의 상세 정보를 조회
	 public Emp findEmpDetailsByEmpno(int empno) {
	     List<Emp> results = jdbcTemplate.query(
	         "SELECT * FROM emp WHERE empno = ?",
	         new Object[]{empno},
	         (rs, rowNum) -> new Emp(
